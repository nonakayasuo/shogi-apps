...
export function make_board(
  ...
  if (
    ...
    for (let i = 0; i < Setting.LENGTH; ++i) {
      squares[i][2] = Fu(false);
      squares[i][6] = Fu(true);
    }
    squares[0][0] = Kyo(false);
    squares[8][0] = Kyo(false);
    squares[0][8] = Kyo(true);
    squares[8][8] = Kyo(true);

    squares[1][0] = Kei(false);
    squares[7][0] = Kei(false);
    squares[1][8] = Kei(true);
    squares[7][8] = Kei(true);

    squares[2][0] = Gin(false);
    squares[6][0] = Gin(false);
    squares[2][8] = Gin(true);
    squares[6][8] = Gin(true);

    squares[3][0] = Kin(false);
    squares[5][0] = Kin(false);
    squares[3][8] = Kin(true);
    squares[5][8] = Kin(true);

    squares[1][1] = Kei(false);
    squares[7][7] = Kei(true);

    squares[7][1] = Hisya(false);
    squares[1][7] = Hisya(true);

    squares[4][0] = Gyoku(false);
    squares[4][8] = Gyoku(true);
  } else {
  }
  return squares;
}
...
export function set_piece(n: number, is_black: boolean): Piece {
  let piece: Piece = Mt();
  if (n === 0) piece = Hisya(is_black);
  if (n === 1) piece = Kaku(is_black);
  if (n === 2) piece = Kin(is_black);
  if (n === 3) piece = Gin(is_black);
  if (n === 4) piece = Kei(is_black);
  if (n === 5) piece = Kyo(is_black);
  if (n === 6) piece = Fu(is_black);
  return piece;
}
...
function control_pos_sub(
  ...
  if (num === 5) {
    ...
    while (0 <= yy && yy < Setting.LENGTH) {
      ...
      // ただし相手玉なら終わらない
      if (
        pos[x][yy].piece_num() !== Setting.MT &&
        !(pos[x][yy].piece_num() === 7 && pos[x][yy].turn() !== turn)
      ) {
        break;
      }
      yy = turn ? yy + dy : yy - dy;
  ...
  if (num === 1 || num === 9) {
    ...
    for (let i = 0; i < dx.length; ++i) {
      ...
      while (0 <= xx && xx < Setting.LENGTH && 0 <= yy && yy < Setting.LENGTH) {
        ...
        // ただし相手玉なら終わらない
        if (
          pos[xx][yy].piece_num() !== Setting.MT &&
          !(pos[xx][yy].piece_num() === 7 && pos[xx][yy].turn() !== turn)
        ) {
          break;
        }
        xx += dx[i];
...
function mate(pos: Piece[][], captured: number[], turn: boolean): boolean {
  ...
  for (let i = 0; i < dx.length; ++i) {
    ...
    // 自分の駒があるとダメ
    if (
      tmp_pos[xx][yy].piece_num() !== Setting.MT &&
      tmp_pos[xx][yy].turn() === turn
    ) {
      continue;
    }
    // 逃げられるマスがあれば詰んでいない
  ...
  for (let i = 0; i < Setting.LENGTH; ++i) {
    for (let j = 0; j < Setting.LENGTH; ++j) {
      // 自分の駒かチェック
      if (
        tmp_pos[i][j].turn() !== turn ||
        tmp_pos[i][j].piece_num() === Setting.MT ||
        tmp_pos[i][j].piece_num() === 7
      ) {
        continue;
      }
...
export class Game extends React.Component<IGameProps, IGameState> {
  ...
  handleClick(i: number) {
          ...
          for (let x = 0; x < Setting.LENGTH; ++x) {
            for (let y = 0; y < Setting.LENGTH; ++y) {
              ...
              if (tmp_pos[x][y].piece_num() !== Setting.MT) {
                ...
                tmp_control_piece[x][y] = turn
                  ? y > 1
                  : y < Setting.LENGTH - 1 - 1;
              } else if (num === 5) {
                tmp_control_piece[x][y] = turn
                  ? y !== 0
                  : y !== Setting.LENGTH - 1;
                ...
                if (
                  turn
                    ? y > 1 &&
                      tmp_pos[x][y - 1].piece_num() === 7 &&
                      tmp_pos[x][y - 1].turn() !== turn
                    : y < Setting.LENGTH - 1 &&
                      tmp_pos[x][y + 1].piece_num() === 7 &&
                      tmp_pos[x][y + 1].turn() !== turn
                ) {
                  if (
                    mate(
                      tmp_pos,
                      turn ? current_white_piece : current_black_piece,
                      !turn
                    )
                  ) {
                    tmp_control_piece[x][y] = false;
    ...
    if (this.state.clicked_piece === Setting.UNCLICKED) {
      // 手番の駒以外はダメ
      if (
        current_pos[x][y].piece_num() === Setting.MT ||
        current_pos[x][y].turn() !== turn
      ) {
        return;
      }
      // 候補の列挙
      ...
      for (let xx = 0; xx < Setting.LENGTH; ++xx) {
        for (let yy = 0; yy < Setting.LENGTH; ++yy) {
          ...
          // 手番の駒はダメ
          if (
            current_pos[xx][yy].piece_num() !== Setting.MT &&
            current_pos[xx][yy].turn() === turn
          ) {
            continue;
          }
    ...
    if (
      ...
      (turn
        ? y < 3 || yy < 3
        : y > Setting.LENGTH - 1 - 3 || yy > Setting.LENGTH - 1 - 3) &&
      num < Setting.WHITE &&
      Setting.PIECES[num + Setting.MT / 2] !== ""
    ) {
      // 必ず成るときは聞かずに成る
      // 歩、香の1段目、桂の1,2段目
      if (5 <= num && num <= 6) {
  ...
  setBoard(target_moves: number) {
    ...
    while (cur < target_moves) {
      ...
      const y = (i - Setting.WHITE * 2) % Setting.LENGTH;
      const xx = Math.floor(
        (clicked_piece - Setting.WHITE * 2) / Setting.LENGTH
      );
  ...
  render() {
    ...
    const legend = this.state.is_black
      ? `△${this.state.white_name}　▲${this.state.black_name}`
      : `▲${this.state.black_name}　△${this.state.white_name}`;
    ...
    const back10 =
      this.state.moves === -1 && moves > 0 ? (
        ...
        <Button
          variant="contained"
          disabled
          onClick={() => this.setBoard(moves - 10)}
        >
          <Replay10OutlinedIcon fontSize={font_size} />
        </Button>
      );
    const back1 =
      this.state.moves === -1 && moves > 0 ? (
        ...
        <Button
          variant="contained"
          disabled
          onClick={() => this.setBoard(moves - 1)}
        >
          <ArrowBackIosIcon fontSize={font_size} />
        </Button>
      );
    const forward1 =
      this.state.moves === -1 && moves < this.state.moves_max ? (
        <Button variant="contained" onClick={() => this.setBoard(moves + 1)}>
    ...
    if (is_mobile) {
      return (
        <div className="game-info">
            ...
            {this.state.moves === -1 && moves_max > 0 && (
              <div className="mobile-button">
                <Button
                  ...
                  size={button_size}
                  color="inherit"
                  onClick={() => this.setBoard(moves_max)}
                >
                  終局図
                </Button>
              </div>
            )}
            <div className="center bold">{legend}</div>
          ...
          <Alert
            title={`まで${moves}手にて${
              this.state.turn ? this.state.white_name : this.state.black_name
            }の勝ちです！`}
            message={""}
